...
class InvertedPendulumEnv(gym.Env):
    ...
    def __init__(self, masscart=0.5, masspole=0.2, total_length=0.6, tau=0.02, task="balance"):
        # set task
        self.task = task
        ...
        self.b = 0.1
        self.n_coords = 2
        # self.d = self.inertial * (self.masspole + self.masscart) + self.masscart * self.masspole * self.l * self.l
        # self.A = np.array(
        #     [0, 0, 1, 0],
        #     [0, 0, 0, 1],
        #     [0, self.g * ((self.masspole * self.l) ** 2) / self.d, - self.b * (self.inertial + self.masspole * (self.l ** 2)) / self.d, 0],
        #     [0, self.masspole * self.g * self.l * (self.masspole + self.masscart) / self.d, - self.masspole * self.l * self.b / self.d, 0]
        # )
        # self.B = np.array(
        #     [0],
        #     [0],
        #     [(self.inertial + self.masspole * self.l * self.l) / self.d],
        #     [self.masspole * self.l / self.d]
        # )
    ...
    def step(self, action):
        ...
        #     reward = 0.0
        reward = x * x + (th - np.pi) * (th - np.pi) + x_dot * x_dot + th_dot * th_dot + (u * u).tolist()[0]
